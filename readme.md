...
In the lstm folder, we show the codes for the implementation of section 6.1 (Validation of STL RPRV Methods) and section 6.2 (Validation of STREL RPRV Methods): 
...
3. In `step_3_alpha_calculation.py`, we calculate the normalization constants alphas for the interpretable methods for STREL RPRV in Section 6.2.
4. In `step_4_distribution_shift_computation.py`, we compute the tuning parameter epsilon for Section 6.2.
5. In `step_5_experiments.py`, we conduct the RPRV experiments listed in Section 6.2 on a given number of agents (adjustable in the params file).
6. In `step_6_plot.py`, we make the plots for the paper and analyze the data (including recorded computation times), etc.
6. In `step_7_alpha_calculation_STL.py`, we show the robust semantics calculation of STL and calculate the normalization constants for the interpretable methods for STL RPRV in Section 6.1.
8. In `step_8_distribution_shift_computation_STL.py`, we compute the tuning parameter epsilon for Section 6.1.
9. In `step_9_experiments_STL.py`, we conduct the RPRV experiments listed in Section 6.1 on a single selected agent.
10. In `step_10_plot_STL.py`, we make plots for the paper specifically for Section 6.1.
...
12. In `step_12_compare_predictors.py`, we implement the codes for Section 6.3 where we compare the effect of predictors on the verification results.

Note that apart from the lstm folder, we also have two other folders (aside from auxiliary_codes), cnn and transformer. The purpose of these folders is to contain codes used to compute the alphas and epsilons necessary for the predictor comparison in Section 6.3. Note that the predictor training codes for the transformer are in `transformer/step_2_predictor_training.py`.
...
First to replicate the results in Section 6.2 (with 5 agents), we recommend the following procedure:
...
To replicate the results in Section 6.1, we recommend the following procedure:
...
### Section 6.3:
To replicate the results in Section 6.3 we reommend the following procedure:

1) Run the aforementioned procedure with 5 agents for section 6.2 with the lstm.
