    ...
    for a in range(params.num_agents):
        ...
        for s in range(3):
            trained_cnn_models[a][s] = keras.models.load_model(
                f"predictors/{params.num_agents}-agent/cnn/cnn_model_agent_{a}_state_{s}.keras")
    print("=== Finished Loading the CNN Model ===")
    predicted_trajectories = step_2_predictor_training.generate_pred_trajectories(trained_cnn_models, trajectories, cnn_norm)
    predicted_shifted_trajectories = step_2_predictor_training.generate_pred_trajectories(trained_cnn_models, shifted_trajectories, cnn_norm)
